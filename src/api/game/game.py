...
async def create(headers:User):
    ...
    #logger.info('/game/create SQL: ' + sql) 
    ...
    query = f'INSERT INTO games(game_id, user_id, word) VALUES("{game_id}", "{user_id}", "{random_word}")'
...
async def get_all_games(headers: User):
    ...
    db = await _get_db()
    sql = f'SELECT game_id FROM games WHERE user_id LIKE "{user_id}" AND finished=0'
...
#async def get_game(id:int):
    ...
    db = await _get_db()
    sql = f'SELECT * FROM games WHERE game_id="{game_id}" AND user_id="{user_id}"'
...
async def make_guess(data:Guess, headers:User):
    ...
    sql = f'SELECT * FROM games WHERE game_id="{game_id}" AND user_id="{user_id}" AND finished=0'
